...
public class TestLiftAuto extends LinearOpMode implements OpModeAddition {
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        waitForStart();
        Movement.setPowerForced(0.5f);
        clk.reset();
        while (clk.milliseconds() < 500){
            telemetry.addData("f",Robot.Motors.LB.getCurrentPosition());
            telemetry.addData("f2",Robot.Motors.RF.getCurrentPosition());
            telemetry.addData("f3",Robot.Motors.RB.getCurrentPosition());
            telemetry.addData("f4",Robot.Motors.LF.getCurrentPosition());
            telemetry.update();
        }
        Movement.setPowerForced(0);
        clk.reset();
        while (clk.milliseconds() < 5000){
